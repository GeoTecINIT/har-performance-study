...
from lib.pipeline.data_loading import load_data
from lib.pipeline.data_grouping import generate_lno_group, generate_training_and_test_sets
from lib.pipeline.data_reshapers import get_reshaper
from lib.pipeline.training import create_trainer
from lib.pipeline.training_report import generate_report, report_writer
from lib.pipeline.utils import set_seed
...
def train_models(data, subjects, test_subjects, model_type, batch_size, epochs, n_splits, reports_dir, testing_mode):
    ...
    try:
        for test_subject in test_subjects:
            with alive_bar(len(subjects) - 1, title=f'Evaluating models with {test_subject}', force_tty=True) as progress_bar:
                        ...
                        for source, (x, y) in data.items():
                            ...
                            if not testing_mode:
                                if not source in writers:
                                    writers[source] = report_writer(os.path.join(reports_dir, '{0}_models.csv'.format(f'{model_type}_{source}')))
